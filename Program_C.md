...
- **`const` 在 `*` 左边**（`const int * p`）：限制的是**内容**。指针是常量。
- **`const` 在 `*` 右边**（`int * const p`）：限制的是**指针**。内容是常量。
...
在嵌入式的领域内数据的存储是使用小端的方式进行存储的，数值：`0x12345678`
...
| :-------: | :--------: | :--------: | :-----------------: | :----------------: |
|  **下标法**  |   `p[i]`   | `*(p + i)` |    ⭐⭐⭐⭐⭐ (应用层首选)    |    一般 (每次需计算偏移)    |
| **指针偏移法** | `*(p + i)` | `*(p + i)` |    ⭐⭐⭐⭐ (底层驱动常用)    |    一般 (每次需计算偏移)    |
| **指针自增法** |   `*p++`   | 读取后地址寄存器自增 |   ⭐⭐⭐⭐⭐ (底层性能榨干)    | 🏆 **极高 (硬件一步到位)** |
| **交换律法**  |   `i[p]`   | `*(i + p)` | ❌ **禁止使用** (仅作八股了解) |         一般         |
...
memset(..., '1', ...) → 写进去的是字符 '1' 的编码值，通常是 0x49
