...
```
### 1.10.3 `return` 的三大工业级应用形态

`return` 在业务层扮演着三种截然不同的角色[[#3.1.2 返回值]|[跳转]]  [跳转](#312-返回值)
...
16位系统：x=2，32位系统：x=4，64位系统：x=8。`uint8_t*` 是**指针**，`sizeof(uint8_t*)` 计算的是“地址本身需要多少字节保存”，不是 `uint8_t` 的大小，所以大小对应的是地址的大小，32位的系统对应4个字节
...
需要注意的是`void* p`是空白指针,表示通用地址，不能进行标准的p++
```text
void * = 通用地址
       ↓
只能保存地址
...
当定义一个数组 `int arr[5];` 时，内存里发生了两件事：1.分配空间： **系统划出了一块连续的内存（能存5个整数）**。2.生成标签：**`arr`这个名字，其实就代表了这块内存首元素的地址**。数学公式：`arr≡&arr [0]` (数组名等于第0个元素的地址)
...
`int *p;`意思是： `p` 是一个装地址的变量。但是有一个问题：如果我现在还不希望 `p` 指向任何东西怎么办？就可以：
...
表示： “我只关心命令成功没成功，我不需要返回字符串和长度。”因此 NULL 也是一种非常重要的**接口设计工具**。
...
`char *p = "hello";`不是： 创建了一个字符串 `p`

`p` 是：`char *`也就是**指针**。而真正的字符串是："hello"

因此更准确地拆开看：`const char *p = "hello";`
...
“换芯测试法”如果在一个函数前犹豫不决，不知道该把它归到 App 还是 Driver，请在脑子里做这个著名的行业测试： **“假设明天老板为了省钱，把物理屏幕从 OLED 换成了 LCD，或者把控制器芯片换了。我手上的这个函数需要修改吗？”**
