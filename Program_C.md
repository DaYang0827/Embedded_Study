...
区分`++`的**前置与后置**
 ...
 后置`p++`规则：**当前表达式使用旧值，然后 p 自增**。可以理解为：
...
==一个指针一次只能取出来一个字节的内容==，因为每个地址都只能存放一个字节的内容，所以指针也只能对应的取出一个字节的内容
...
数组名虽然可以代表了这块内存首元素的地址，但是不能对数组名进行自增操作
