...
我现在只想操作 `bit3`，那我可以准备一个：`0000 1000` 这个数就是一个**掩码**。因为它只有 bit3 是 `1`：
...
所以它相当于在告诉程序： “其他位我都不关心，我只盯着 bit3。”
...
数组名 `a` 在绝对不可修改这一点上，本质上等价于一个 **‘指针常量’（Constant Pointer）**。也就是类似于 **`uint8_t * const ptr`**。这种指针的特点是，指针本身存储的地址一旦初始化就死死锁定了，绝对不允许执行 `ptr++` 或者 `ptr = xxx` 来修改它指向别人，但它指向的格子里面的内容（如 `a[0] = 0x99`）是可以被修改的。
...
编译器可能说： 不合适，我还是正常调用吧。所以：inline ≠ 强制展开
