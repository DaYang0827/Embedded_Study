...
各是什么。
```text
a是定义的uint32_t的变量    存放了10
p是定义的uint32的指针    里面应该存放一个地址     步长是4个字节
*p是取出指针对应的内容     这里面就是10
pp是二级指针    里面存放一级指针的地址
*pp 就是p  里面是a的地址
**pp 取出来是a的值   就是10
...
`volatile` 能不能保证原子性？
```text
经常用volatile是为了防止系统过于优化一些变量      
有一些变量比如寄存器里面的饿某一位是中断的标志位    需要时刻观察他的变化    来判断是否进入中断    需要用到volatile
但是不能保证原子性     原子性是一个操作是否会被打断     用volatile修饰并不能保证原子性
```
...
它底层大概经历哪几个步骤？
```text
这个需要把count的值取出来   再加一  再存回去    中间有中断的时候是会被打断的
```
...
```text
不会
因为如果主循环再执行自增的时候突然进入中断    会打断主循环这个操作    在中断中再执行数值可能就会有问题
```
...
```text
先把1左移三位    就是0000100
reg = reg | 000000100   因为是或运算   只要有一个1就会置一    所以可以进行置一操作
先把reg的值读出来    再进行或操作改写     再写回去
```
...
每一步在做什么。
```text
先把1左移三位      然后进行~操作   进行反转变成0
再进行与操作    与需要两个1才置一   有一个零就是零     可以进行清零操作
```
...
c在.bss里面   是static修饰的
d也在.data里面
e在stack里面  是函数内部没有初值的变量
f在.bss里面    虽然在函数内部但是有static修饰    生命域是整个程序
```
10. 为什么全局变量：
...
```text
全局变量默认为0    是放在.bss里面的
但是函数内部的变量   是存放在栈里面的     如果之前栈里面有数据    就会直接把数据给到a    从而发生错误
```
...
访问它？
```text
有static进行修饰    改变了作用域   只能在本.c文件里面进行调用    禁止外部extern
```
...
什么时候真正分配内存？
```text
是声明变量     代表着在这个工程文件的别的.c文件里面有这个变量的定义
对应的内存分配是在对应的.c文件编译的时候
```
...
```text
主要是进行改变改变生命周期和作用域的
如果是函数内部的static就是改变生命周期    延长至整个程序周期
如果是全局变量  就是改变作用域    可以让别的.c文件也能extern调用到变量
```
...
```text
他的初始值最开始的时候是编译的时候    直接放到了flash里面的.rodata里面的    当程序正式开始运行的时候（rest handler）执行的时候   会把flash的rodata数据搬移到.data里面进行运行
```
...
它主要存什么？
```text
stack是栈    是位于sram里面的一块特殊区域    是从上往下生长的     遵循先进先出的规则
里面主要存的都是函数执行的时候产生的数据
以及对应的保存的进入退出函数的指针
```
19. 什么是 Heap？
...
```text
因为动态内存分配需要进行手动的free释放掉这些内存
如果没有释放   就会发生内存泄漏    也容易造成内存不够的情况
```
...
请解释这两行分别表示什么。
```text
第一行代表msp保存的地址是0x200006A8  代表的是主指针是从0x200006A8这个ram区域开始执行的
第二行也是   代表resethandler是从0x08010229开始执行的
```
26. 为什么 `Reset_Handler = 0x08010229` 是奇数？

最低位的 `1` 表示什么？
```text
最低位代表的是用thumb形式      对应的应该是偶数地址即0x08010228
```
...
```text
虽然app的msp已经是在0x08010000但是还不知道里面具体都装了什么地址    如果APP的数据传输有问题   或者出现什么错误   可能会导致里面还全是0xFFFFFFFF的情况    肯定是有问题的     或者里面存放的数据不是在合理的sram区间里面也是非法的
同理reset handler也是这样的考虑
```
28. CRC 校验成功以后，为什么还不能直接说明 APP 一定能运行？
```text
crc校验成功    不一定APP就一定能运行     因为可能会出现上位机就出错的情况    比如对应的msp没有定位到sram的合法区间   等情况
```
...
```text
expectcrc是上位机进行计算的
actualcrc是Bootloader接收到数据包之后计算的
因为在固件传输的时候可能发生错误等情况     需要两边都计算crc最后检查两个值是否相同    来判断是不是没有发生错误
```
...
为什么这个校验毫无意义？
```text
因为这个判断条件是自身的crc与自身的crc进行比较     一定是相同的     所以校验没有意义
```
31. 你现在协议是：
...
分别解释每一部分作用。
```text
AA和55都是代表的开头    只有在同时收到AA 55之后才能判断这个是数据包的开头
len代表的是这个数据包data的长度
cmd对应的是想要进行的命令指令
data就是对应的想要传输的数据
sum是len+cmd+data的第八位的值    想要检查这个数据帧在传输的过程中是否有问题    需要Bootloader自己先计算一次sum  再和这个sum进行比较
```
...
```text
判断空用read == write代表RingBuffer里面还都是空着    所以这两个才会相同
满了应该是（write + 1） % （sizeof（RingBuffer）） == read  这个代表的是写入的下一位就会追到读指针    RingBuffer会少利用一位来判断是不是已经满了 
```
...
你怎么回答？
```text
因为在数据帧的开头就会遇到需要分情况的情况   比如0xAA   下一位不是0x55所以使用状态机是最方便的进行判断
同时在cmd进行解析的时候   也需要针对cmd的数据进行分情况执行    最好就是直接使用状态机
```
...
```text
“直接 memcpy”不是一定不能判断 LEN/SUM，而是它通常要求你：
...
状态机应该怎么处理比较合理？
```text
状态机在接收到第二个AA的时候    重新跳转状态到wait 55  代表重新开始数据帧的写入
```
...
为什么要立刻丢包？
```text
因为这个包是不合规的       及时后面的内容写进来   也会运行不了    所以直接丢包
```
...
```text
以为APP程序可能也需要用到这些外设或者是中断
如果在跳入app前不进行处理
那么APP执行的时候     可能遇到中断和外设就会去Bootloader的对应接口里面执行
```
...
怎么定义parser的状态机     去接收接收到的数据帧
怎么定义cmd的状态机   从而把接收到的数据对应成操作码
```
...
你怎么诚实又有竞争力地回答？
```text
这个Bootloader我是通过chat辅助我进行编程的
我先确定了我的项目的主体方向    再让chat给我每一个小点的对应操作
慢慢把所有的小点合在一起
在过程中再慢慢的调试   和对项目的升级
```
...
```text
主要还是针对指针的理解    以及结构体和状态机的理解
对于地层   知道了具体的内存分配都是啥样的    flash是怎么划分区域的   以及ram和flash里面都存什么   都是怎么进行数据保存和调用的
协议主要就是两个状态机的搭配    把接收到的数据帧进行分析
flash里面知道了flash里面存储的内容   是怎么进行擦写的以及是怎么划分区域的
调试主要是每一个小功能都进行单步调试     去watch里面观察具体都是怎么实现的    有问题又体现在哪里   几个小功能是怎么一起搭配协同工作的
架构方面就是先分析这个项目的要求    再抽象成每一块的功能    再到具体的实现
```
...
```text
在metadata里面保存之前的version号码
如果发生了断电中断    就重新进入Bootloader   不进行跳转通过版本号来重新再进行烧录
```
...
为什么不能？
```text
不能     因为如果发生了恶意篡改   可能在上位机里面就把数据更改了     导致上位机计算的expect crc就是有问题的    对应算出来的actual 虽然也能相同但是已经被修改了
```
