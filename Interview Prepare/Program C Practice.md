...
```text
.data里面有初始化为零的变量   还有固定初始化的变量   或者有static修饰的有初值的局部变量
.bss里面放的都是没有初始化的全局变量   和有static修饰的没有初值的局部变量
```
...
和 Stack 最大区别是什么？
```text
heap是堆    最大的区别是和stack是对向生长的   从下向上生长的
```
...
```text
因为可能会出现aa 55 aa这种情况    需要重新判断数据帧的开头   如果使用memcpy就会把所有的内容全填入进去    就会收到错误的数据帧    而且也没办法判断数据的len会不会超过size大小   以及没有sum进行判断这个数据帧传输是否有问题
```
