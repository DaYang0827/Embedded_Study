...
后果：如果写了 `int a`; 并且假设它是 16 位的，当你把代码从 Arduino 移植到 STM32 时，内存占用突然翻倍了；反之，如果从 STM32 移植到 8 位机，原本能存 100000 的变量突然溢出了（因为 16 位最大只能存 32767）。
...
是给外部模块使用的公共接口。而`static void uart_gpio_init(void);` 只是 `uart.c` 内部的辅助函数，不应该暴露出去。这样做可以防止其他文件错误调用，避免不同文件中同名函数发生冲突，让代码接口更加清晰

### 1.4.4 static 与静态分配的不同
...
这里`#define BUFFER_SIZE 128U`只是告诉预处理器：以后看到 `BUFFER_SIZE` 就替换成 `128U`。而
`static uint8_t rx_buffer[128];`是**真的在内存里分配了 128 字节空间。**
...
在这里的`p`表示的是： **整个 `Point_t` 结构体对象的数据 / 值** 而不是 `p` 的地址。所以`return p;`  就是：**返回整个结构体的值。
...
快慢指针是**用两个变量记录两个不同的“当前位置”，按照不同规则移动。** 最典型的是读写指针，这个例子就是进行删除数组中指定元素的操作
...
`read`  相当于“快指针”，负责：**不断往前扫描原数组**。而：`write`   相当于“慢指针”，负责： **记录下一个有效元素应该写到哪里。**
...
**野指针 wild pointer：** 通常指：`int *p;`这种没有初始化、里面地址不确定的指针。

```text
p → ??? 随机位置
```

**悬空指针 dangling pointer：** 曾经合法，但目标对象已经失效。
